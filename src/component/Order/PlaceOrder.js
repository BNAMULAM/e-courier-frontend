...
function PlaceOrder() {
  ...
  const addOrder = async (e) => {
    ...
    if (!mobileNo) {
      ...
      validationErrors.mobileNo = "Mobile Number Must Begin With 7,8,9";
    }
    if (!fromLocation) {
    ...
    if (Object.keys(validationErrors).length === 0) {
      ...
      await axios
        .post("http://localhost:8082/order/place", order)
        .then((Response) => {
          console.log(Response.data);
          
          alert("Order Placed Successfully Your Tracking Number Is : "+Response.data.trackingNumber);
        })
        .catch((err) => {
          console.error(err);
          alert("Error Occured");
    ...
    <div>
      ...
      <div className="cls1">
        ...
        <form>
          ...
          />
          {errors.fromLocation && (
            <p className="text-danger">{errors.fromLocation}</p>
          )}
          <br />
          ...
          />
          {errors.fromPinCode && (
            <p className="text-danger">{errors.fromPinCode}</p>
          )}
          <br />
          ...
          />
          {errors.toLocation && (
            <p className="text-danger">{errors.toLocation}</p>
          )}
          <br />
          ...
          />
          {errors.toPinCode && (
            <p className="text-danger">{errors.toPinCode}</p>
          )}
          <br />
          ...
          <button className="btns" type="button" onClick={addOrder}>
            Place Order
          </button>
